...
- 2026-09-20T21:27:48Z [id:c5a172cae6d0292f406625e59df9ead86f3bd7f63f41fa13bdf32bc14c7fea2f] ## add-evals-guidelines change scaffolded (2026-09-20)
  
  - openspec/changes/add-evals-guidelines/ scaffolded + validated --strict, 0/17 tasks: guidelines-first reframe of the deleted add-live-replay scaffold — genesis owns conventions (tier ladder, process-boundary scoring, authoring standard, live action protocol, :free entry tier, sandbox, versioned report contract, fault routing); consumer repos own their batteries/harnesses. Live-runner deliberately out (revisit trigger: demonstrated duplication across >=2 tools). Shared code artifact: only ErrorTaxonomy::ActionFormatViolation (ERR_ACTION_FORMAT_VIOLATION).
  - Rule-of-5 review on the specs found 1 CRITICAL (report contract required model id for all tiers vs tier-0/1 having none), 1 metric-loophole HIGH (empty-trajectory pass), routing key/threshold fix (channel not scenario; all-configured-ids fallback). All fixed in spec+proposal+design+tasks; validate --strict green.
  - openspec --strict gotcha: a MODIFIED requirement whose first paragraph is a blockquote before the SHALL sentence fails validation (SHALL must be in the parsed body).
