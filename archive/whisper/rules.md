...
### `skills` CLI (Vercel agent-skills) — flag semantics
`npx skills add <owner>/<repo>` installs interactively; the real flags are `-a agent1,agent2` (agents, comma-separated), `-s skill1,skill2` (skills), `-g` user-level, `--copy` instead of symlinks, `-l` list-only. **Trap:** `-l` prints an "installing non-interactively" banner yet performs *no* install — it is safe read-only. Skills land as symlinks into each detected agent's directory (e.g. `~/.claude/skills`); default scope is the current project, not global. (Verified 2026-09-05 against `skills add --help` while documenting PhormaSci/skills — the README flags I had written from memory were wrong.)

### Acceptance-test skill/reference code blocks verbatim, in a clean shell
Skill files are agent-executed code — before committing one, extract the block verbatim from the file and run it under `env -i HOME="$HOME" bash -c '...'` (or with the target repo's real state). Two broken implementations of the same resolver passed casual review and were only caught by this: a sed comment-strip that couldn't handle column-aligned inline comments in `.bashrc`, and `JOURNAL_PATH="$JORNAL"` evaluating empty because the referenced var doesn't exist in non-interactive shells. Method: `sed -n '/start/,/end/p' file | sed 's/^   //'` → source it. (Established incitaciones close-skill work, 2026-09-05.)
...
- 2026-09-18T21:33:41Z [id:ac86d0e4ba57832f7ad543abc3ad9540db99bcb22efaee71fc29b059e1cc1051] (#bd-dolt) bd 1.0.4 'bd dolt push' to git+ssh remotes is unreliable: dolt's git-remote-cache dies with 'git rev-parse ... must be run in a work tree' and the URL loses its user part (exit 128); even healthy repos like wai show 'no common ancestor' vs their refs/dolt/data. Working pattern (dont): config no-db true → .beads/issues.jsonl is source of truth; git tracks only .beads/config.yaml + issues.jsonl (root .gitignore needs '.beads/*' with negations because global ~/.gitignore ignores .beads/); keep local embeddeddolt/ untracked — deleting it breaks bd until 'bd init --from-jsonl' restores it. Custom GIT_SSH_COMMAND breaks dolt pushes; dolt resolves ssh-config aliases itself.

- 2026-09-20T17:02:00Z [id:pi-orchestration-close] (#pi-orchestration) Orquestación de sesiones pi headless (`pi --session-id <id> --name <n> -p < prompt`): (1) reusar el mismo --session-id en una re-invocación hace APPEND del prompt a la sesión existente y puede abortar sin procesar nada — para reintentar, usá un id nuevo e incrustá el contexto destilado del transcript (`~/.pi/agent/sessions/<dir-slug>/<ts>_<id>.jsonl`). (2) "Connection error" o "Command aborted" al final de `pi -p` NO implica fallo — verificar el estado real (bd show, git log/push, suite) antes de re-lanzar. (3) Prompt de orquestación por ticket: incluir claim ya tomado (o el agente aborta), pipeline del repo, y verificar gates entre tickets.

- 2026-09-20T21:52:00Z [id:typos-config-layers] typos config layers: (1) lowercase tokens (e.g. column names `cacl`, `otu`) are matched through `[default.extend-words]`, NOT `[default.extend-identifiers]` — identifiers only catch cased tokens (video IDs, acronyms); verify with `typos --dump-config -` that the config actually loads. (2) typos applies config excludes only during directory walks — explicitly-passed file args bypass exclusion, so a lefthook-style staged-file pipeline needs its own grep filters for out-of-scope content (es trees, session notes). (3) beads' `core.hooksPath=.beads/hooks` chains to lefthook — when debugging hook behavior, check which wrapper actually fires. (talleres session, 2026-09-20.)
